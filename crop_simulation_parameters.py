...
def store_netcdf(nc_ds: xr.Dataset, file: pl.Path) -> None:
    file_tmp = file.with_suffix(".tmp.nc")
    file_tmp.parent.mkdir(parents=True, exist_ok=True)
...
def crop_pcraster_file(
    ...
    store_pcraster(pcr_map=domain_map, file=file_out)
...
def crop_netcdf_file(
    ...
    domain_ds = ds.sel(
        {
            lat_name: slice(domain_north, domain_south),
    ...
    times = ds.time
    domain_times = pd.Series(
        pd.date_range(start=domain_start, end=domain_end, freq="D")
    )
    if len(times) == 1:
        store_netcdf(nc_ds=domain_ds, file=file_out)
        return
...
for simulation in simulations:
    ...
    for index, line in enumerate(lines):
        ...
        if "%04d" in pcr_file.name:
            for level in levels:
                prc_name_level = pcr_file.name.replace(
                    "%04d", f"{level*100:04.0f}"
                )
    ...
    for index, line in enumerate(lines):
        ...
        if "%04d" in nc_file.name:
            for level in levels:
                prc_name_level = nc_file.name.replace(
                    "%04d", f"{level*100:04.0f}"
                )
